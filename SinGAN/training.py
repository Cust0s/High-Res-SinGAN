...
def train(opt,Gs,Zs,reals,NoiseAmp):
    ...
    while scale_num<opt.stop_scale+1:
        ...
        # opt.nfc = min(opt.nfc_init * pow(2, math.floor(opt.stop_scale / 4)), 128)       #for constant #of filters
        # opt.min_nfc = min(opt.min_nfc_init * pow(2, math.floor(opt.stop_scale / 4)), 128)
        opt.scale_num = scale_num
        print("\n",opt.nfc, opt.min_nfc,"#L", opt.num_layer,(opt.min_size,opt.max_size))
...
def train_single_scale(netD,netG,reals,Gs,Zs,in_s,NoiseAmp,opt,centers=None):
    ...
    # for epoch in range(opt.niter-int(opt.scale_num*opt.niter*0.1)): # for epoch decay with constant filters
        ...
        schedulerD.step()
        schedulerG.step()
    plt.plot(errG2plot,'g')
    plt.plot(errD2plot, 'r')
    plt.savefig('%s/loss.png' %(opt.outf,))
    plt.clf()
